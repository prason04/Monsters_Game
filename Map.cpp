...
// if the player character lands on NormalSpace
bool Map::isSpaceLocation(int row, int col)
{
    if (!isOnMap(row, col))
    {
        return false;
    }
    for (int i = 0; i < spaces_count_; i++)
    {
        if (row == space_positions_[i][0] && col == space_positions_[i][1])
        {
            return true;
        }
    }
...
// add NormalSpace location to the map
bool Map::addSpace(int row, int col)
{

    if (spaces_count_ >= max_spaces_)
    {
        return false;
    }
    ...
    space_positions_[spaces_count_][0] = row;
    space_positions_[spaces_count_][1] = col;
    spaces_count_++;
    map_data_[row][col] = SPACE;
    return true;
}

// if the player character lands on NormalSpace
bool Map::isMonsterLocation(int row, int col)
{
    if (!isOnMap(row, col))
    {
        return false;
    }
    for (int i = 0; i < monsters_count_; i++)
    {
        if (row == monster_positions_[i][0] && col == monster_positions_[i][1])
        {
            return true;
        }
    }
...
// add Monster location to the map
bool Map::addMonster(int row, int col)
{
    if (monsters_count_ >= max_monsters_)
    {
        return false;
    }
    if (!isFreeSpace(row, col))
    ...
    monster_positions_[monsters_count_][0] = row;
    monster_positions_[monsters_count_][1] = col;
    monster_positions_[monsters_count_][2] = false;
    monsters_count_++;
    map_data_[row][col] = MONSTER;
    return true;
}